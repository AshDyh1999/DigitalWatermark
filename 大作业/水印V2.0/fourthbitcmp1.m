function  count=fourthbitcmp1(cover,massage,goleimage,goleimage1,permission,permission1,level,threshold)
%提取图像信息并分层
%cover='lena512.bmp';
%massage='yincang.bmp';
...
for i=1:a
    if count(i)>=threshold
        ...
        for m=1:8
            for n=1:8
                ...
                cover2(8*k3+m,8*k4+n)=bitand(cover2(8*k3+m,8*k4+n),240);
                takemsg3(8*k3+m,8*k4+n)=bitand(msg(8*k3+m,8*k4+n),240);
                shiftmsg3(8*k3+m,8*k4+n)=bitshift(takemsg3(8*k3+m,8*k4+n),-4);
                cover2(8*k3+m,8*k4+n)=bitor(cover2(8*k3+m,8*k4+n),shiftmsg3(8*k3+m,8*k4+n));
            end
        end
    ...
    data1(:,:,level)=cover2;
    imwrite(data1,goleimage1,permission);   
%     %提取秘密图像信息,检测隐藏效果
